...
if (!session_id()) {
	...
	nuSetSafeSessionName($nuConfigDBName);
...
function nuSetSafeSessionName($name) {

	global $nuConfigSetSessionName;
	if (isset($nuConfigSetSessionName) && $nuConfigSetSessionName === false) {
		return;
	}

	// allow only letters, digits, underscores
	$safe = preg_replace('/[^a-zA-Z0-9_]/', '_', $name);

	// ensure it starts with a letter
	if ($safe === '' || !preg_match('/^[a-zA-Z]/', $safe)) {
		$safe = "S_$safe";
	}

	// enforce PHP's session name length limit (128 chars)
